...
The model was trained on the [Census Income Adult dataset](https://archive.ics.uci.edu/dataset/20/census+income) from the UCI Machine Learning Repository, which contains demographic and employment attributes such as age, workclass, education, marital status, occupation, race, sex, and native country fromn a 1994 census extract. Categorical features were one‑hot encoded, and the target label (salary) was binarized. An 80/20 train–test split was used. 

## Evaluation Data 

Evaluation was performed on the held‑out test split from the same dataset. Slice‑based evaluation was conducted on the test set across categorical features, with results written to slice_output.txt. No additional external validation data was used.

## Metrics 

The model was evaluated using precision, recall, and F1 score, consistent with the project rubric. The model’s performance on the test set reflects a precision‑oriented classifier that identifies high(ish)‑income individuals conservatively while missing a portion of true positives.
...
Higher‑education and professional‑occupation groups show stronger recall, while lower‑education and service‑related slices exhibit reduced performance, reflecting underlying dataset imbalances. Slices with clearer income patterns achieve higher F1, while lower‑education groups show lower overall performance.
